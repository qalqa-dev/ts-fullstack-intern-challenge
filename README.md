...
`.env` файл я специально убрал из gitignore, чтобы выполнялось условие с запуском через `docker-compose` up. Из него убрал только api-key, т.к его я показывать не хочу, но чтобы работали картинки пород, его туда нужно вписать, дальше все будет работать как и должно.
